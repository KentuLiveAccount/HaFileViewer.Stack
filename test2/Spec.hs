...
import HaFileViewer.IndexedFileReader
...
spec :: Spec
spec = describe "HaFileViewer.IndexedFileReader (Unified Index)" $ do
  it "returns correct slice and supports negative indexing" $
    withSystemTempDirectory "hfvt2" $ \dir -> do
      ...
      lm <- openIndexedFileReader fp indexStepDefault
      ls <- getLines lm 1 3
      ls `shouldBe` ["line2","line3","line4"]
      ls2 <- getLines lm (-2) 2
      ls2 `shouldBe` ["line4","line5"]
      closeIndexedFileReader lm
